...
func main() {
  ...
  f, err := os.Open("csv/uni-melb-granger.csv")
  if err != nil {
    panic(err)
  }
  defer f.Close()
  ...
  for hl, line := range lines {
    ...
    if hl > 0 {
      fmt.Println(line[0] + " " + line[1])
      statement, err := env.db.Prepare("INSERT INTO stg_uni_melb_granger (header_identifier , date_latest , metadata_identifier , metadata_identifier_handle_id , metadata_identifier_cms_id , metadata_identifier_accession_id , metadata_identifier_file_id , url ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)")
      if err != nil {
        fmt.Println(err)
        return
      }
